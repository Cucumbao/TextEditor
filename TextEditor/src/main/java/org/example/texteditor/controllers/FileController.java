...
public class FileController {
    ...
    public FileController(SnippetRepository snippetRepository, FileRepository fileRepository,
                          ...
                          BookmarkRepository bookmarkRepository, JavaSyntaxHighlighter syntaxHighlighter) {
    ...
    // --- новий файл ---
    @GetMapping("/new")
    public String newFileForm(HttpSession session, Model model) {
        User currentUser = (User) session.getAttribute("currentUser");
        if (currentUser == null) return "redirect:/";
    ...
    @PostMapping("/new")
    public String createFile(@ModelAttribute File file, HttpSession session) {
        User currentUser = (User) session.getAttribute("currentUser");
        if (currentUser == null) return "redirect:/";
    ...
    // --- перегляд файлу ---
    @GetMapping("/{id}")
    public String viewFile(@PathVariable Long id, HttpSession session, Model model) {
        User currentUser = (User) session.getAttribute("currentUser");
        if (currentUser == null) return "redirect:/";
    ...
    // --- редагування ---
    @GetMapping("/{id}/edit")
    public String editFile(@PathVariable Long id, HttpSession session, Model model) {
        User currentUser = (User) session.getAttribute("currentUser");
        if (currentUser == null) return "redirect:/";
